...
import { useMutateCreateCategory } from "@/features/clubs/hooks/useMutateCreateCategory";
import { generateClientUUID } from "@/shared/utils/uuid";
import { useQueryString } from "@/shared/hooks/useQueryString";

/**
 * @component CreateCategoryModal (Organism)
 * @description Modal para crear una nueva categoría dentro de un club.
 * Se abre cuando `?create_category=true` está en los searchParams.
 *
 * Campos:
...
export default function CreateCategoryModal() {
  ...
  const isOpen = searchParams.get("create_category") === "true";
  ...
  const createCategoryMutation = useMutateCreateCategory(clubUuid);

  /** URL de cierre preservando otros searchParams */
  const closeHref = createQueryString("create_category", null);
  ...
  const handleCreate = () => {
    if (!isValid || createCategoryMutation.isPending) return;
        ...
        <div className="flex flex-col gap-5 px-6 py-5">
          ...
          <button
            ...
            className={`w-full cursor-pointer rounded-xl py-3 text-sm font-extrabold transition-all duration-300 ${
              isValid && !createCategoryMutation.isPending
